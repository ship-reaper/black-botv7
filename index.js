...
const ZeksApi = 'apextus'
const DapApi = 'reaperbotwa'
const OnlyDev = 'YOUR_APIKEY'
const TobzKey = 'BotReAr'
...
//owner number
const ownerNumber = ["523131134857@s.whatsapp.net"]
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('(+)','white'), color('BlackReaper','red'), color('(+)','white'), color(' Black Shadow','aqua'), color('Shadow BlackReaper','yellow'))
})
...
// ANTI LINK GRUP
		        ...
		        if (isGroupAdmins) return reply('Admin Grup Mah :D')
		        dappa.updatePresence(from, Presence.composing)
		        if (mesejAnti.includes(",izinkak")) return reply("Sí hermana, no hagas spam")
           ...
           if (isGroup) {
					try {
						...
						setTimeout( () => {
								...
								setTimeout( () => {
								dappa.updatePresence(from, Presence.composing)
								reply("Espero que tu madre este sana")
							}, 2000)
								setTimeout( () => {
								dappa.updatePresence(from, Presence.composing)
								reply("estoy fuera")
		...
		case 'tes':  
                ...
                dappa.updatePresence(from, Presence.composing)
                if (!isGroup) return reply(ind.group)
                ppUrl = await dappa.getProfilePicture(from) 
		...
		case 'tomp3':
                if (!isRegistered) return reply(ind.noregis())
                	dappa.updatePresence(from, Presence.composing) 
					if (!isQuotedVideo) return reply('❌ responder video um ❌')
					reply(ind.wait())
		...
		case 'opentime': 
              ...
              if (args[1]=="detik") {var timer = args[0]+"000"
				...
				setTimeout( () => {
					...
					const open = {
					text: `*𝙰 𝚝𝚒𝚎𝚖𝚙𝚘* 𝙶𝚛𝚞𝚙𝚘 𝚊𝚋𝚒𝚎𝚛𝚝𝚘 𝚙𝚘𝚛 𝚎𝚕 𝚊𝚍𝚖𝚒𝚗𝚒𝚜𝚝𝚛𝚊𝚍𝚘𝚛 @${nomor.split("@s.whatsapp.net")[0]}\n𝙰𝚑𝚘𝚛𝚊 *𝙼𝚒𝚎𝚖𝚋𝚛𝚘* 𝚙𝚞𝚎𝚍𝚎 𝚎𝚗𝚟𝚒𝚊𝚛 𝚖𝚎𝚗𝚜𝚊𝚓𝚎𝚜`,
		...
		case 'neonime':
			if (!isRegistered) return reply(ind.noregis())
					dappa.updatePresence(from, Presence.composing) 
					data = await fetchJson(`https://api.vhtear.com/neonime_search?query=${body.slice(9)}&apikey=${Vhtear}`, {method: 'get'})
		...
		case 'pasangan':
			if (!isRegistered) return reply(ind.noregis())
				...
				sa = pa.split("/")[0];
				ngan = pa.split("/")[1];
				anu = await fetchJson(`https://api.vhtear.com/primbonjodoh?nama=${sa}&pasangan=${ngan}&apikey=${Vhtear}`, {method: 'get'})
		...
		if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(limitend(pushname2))
                   anu = await fetchJson(`https://api.vhtear.com/cerita_sex&apikey=${Vhtear}`, {method: 'get'})
		...
		if (!isRegistered) return reply(ind.noregis())
			...
			case 'nekojav':
					...
					if (!isRegistered) return reply(ind.noregis())
						...
						dappa.sendMessage(from, buffer, image, {quoted: mek, caption: 'No hagas ingredientes para mi tío'})
					} catch (e) {
			...
			case 'elaina':
					...
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=elainamajonotabitabi&apikey=${Vhtear}`, {method: 'get'})
		...
		if (!isRegistered) return reply(ind.noregis())
			...
			case 'pictlolicon':
					...
					dappa.sendMessage(from, nye, image, { caption: 'HOLA ONII CHAN!!', quoted: mek })
					break
					case 'pictwaifu':
					if (!isRegistered) return reply(ind.noregis())
					if (!isGroup) return reply(ind.group)
			...
			case 'senku':
					if (!isRegistered) return reply(ind.noregis())
                                if (isLimit(sender)) return reply(limits.limitend(pushname2))
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=senku&apikey=${Vhtear}`, {method: 'get'})
			...
			case 'wibu':
				if (!isRegistered) return reply(ind.noregis())
					reply(ind.wait())
					anu = await fetchJson(`https://api.vhtear.com/randomwibu&apikey=${Vhtear}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
...
break
			case 'nhentai':
				...
				husw = body.slice(7)
					reply(ind.wait())
					anu = await fetchJson(`https://api.vhtear.com/nhentaipdfdownload?query=${husw}&apikey=${Vhtear}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
			...
			case 'ramaljadian':
				if (!isRegistered) return reply(ind.noregis())
					...
					var gbl2 = gh.split("|")[1];
					var gbl3 = gh.split("|")[2];
					anu = await fetchJson(`https://api.vhtear.com/harijadian?tgl=${gbl1}&bln=${gbl2}&thn=${gbl3}&apikey=${Vhtear}`, {method: 'get'})
					reply(anu.result.hasil)
					break
			case 'memeindo':  
				if (!isRegistered) return reply(ind.noregis())
		...
		if (!isRegistered) return reply(ind.noregis())
			...
			case 'toxic':
                ...
                imageToBase64(`https://api.vhtear.com/textxgif?text=${args[0]}&apikey=${Vhtear}`).then((response) => {var buf = Buffer.from(response, 'base64');
		...
		case 'shortlink3':
				...
				dappa.updatePresence(from, Presence.composing)
				reply(ind.wait())
				data = await fetchJson(`https://onlydevcity.herokuapp.com/api/short/tiny?url=${args[0]}&apikey=${OnlyDev}`)
				hasil = `link : ${args[0]}\n\nOutput : ${data.result.link}`
				reply(hasil)
				break
		case 'textbyname':			
		...
		case 'sagiri':				
			...
			case 'puisiimg':
                   if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(limitend(pushname2))
                   pus = await getBuffer(`https://api.vhtear.com/puisi_image&apikey=${Vhtear}`, {method: 'get'})
	...
	if (!isRegistered) return reply(ind.noregis())
		...
		case 'dadu':
			...
			case 'asupan':
                ...
                addKoinUser('523131134857@s.whatsapp.net', fee)
			...
			if (args.length < 1) return reply(ind.wrongf())
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					buffer = await getBuffer(`https://api.vhtear.com/silktext?text=${silk}&apikey=${Vhtear}`)
					reply(ind.wait())
		    			dappa.sendMessage(from, buffer, image, {quoted: mek})
		    			await limitAdd(sender)	
		    			break
					...
					teks = `${body.slice(7)}`
					atytyd = await getBuffer(`https://api.vhtear.com/slidingtext?text=${teks}&apikey=${Vhtear}`, {method: 'get'})
					reply(ind.wait)
					...
					cml1 = cml.split("/")[0];
					cml2 = cml.split("/")[1];
					buffer = await getBuffer(`https://api.vhtear.com/logoml?hero=${cml1}&text=${cml2}&apikey=${Vhtear}`, {method: 'get'})
					dappa.sendMessage(from, buffer, image, {quoted: mek})
					await limitAdd(sender) 
					break  
//case DappaUhuy
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				dap1 = ct.split("|")[0];
                dap2 = ct.split("|")[1];
				anu = await fetchJson(`https://dapuhy-apikey.herokuapp.com/api/textmaker?text=${dap1}&text2=${dap2}&theme=glitch&apikey=${DapApi}`)
				...
				reply(ind.wait())
				ct = body.slice(4)
				anu = await fetchJson(`https://dapuhy-apikey.herokuapp.com/api/textmaker/senja?text=${ct}&theme=coffee-cup&apikey=${DapApi}`)
				dapuhy = await getBuffer(anu.result.url)
				dappa.sendMessage(from, dapuhy, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "DapBOT~", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('lib/odc.jpeg')} } }, caption: 'Nih hasilnya kak...'})
				break
				case 'cup2':
				...
				reply(ind.wait())
				ct = body.slice(5)
				anu = await fetchJson(`https://dapuhy-apikey.herokuapp.com/api/textmaker/senja?text=${ct}&theme=coffee-cup2&apikey=${DapApi}`)
				dapuhy = await getBuffer(anu.result.url)
				dappa.sendMessage(from, dapuhy, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "DapBOT~", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('lib/odc.jpeg')} } }, caption: 'Nih hasilnya kak...'})
				break
				case 'battlefield':
				...
				dap1 = ct.split("|")[0];
                dap2 = ct.split("|")[1];
				anu = await fetchJson(`https://dapuhy-apikey.herokuapp.com/api/textmaker/game?text=${dap1}&text2=${dap2}&theme=battlefield&apikey=${DapApi}`)
				dapuhy = await getBuffer(anu.result.url)
				dappa.sendMessage(from, dapuhy, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "DapBOT~", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('lib/odc.jpeg')} } }, caption: 'Nih hasilnya kak...'})
				break
...
break
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
                    ...
                    buffer6 = await getBuffer(`https://api.vhtear.com/padlock?text1=${teks}&text2=${teks}&apikey=${Vhtear}`, {method: 'get'})
...
break
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					pinte = body.slice(11)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=${pinte}&apikey=${Vhtear}`, {method: 'get'})
					reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					anu = await fetchJson(`https://api.vhtear.com/randomloli&apikey=${Vhtear}`, {method: 'get'})
					reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					anu = await fetchJson(`https://api.vhtear.com/randomnekonime&apikey=${Vhtear}`, {method: 'get'})
					reply(ind.wait())
					buffer = await getBuffer(anu.result.result)
			...
			case 'waifu2':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					data = await fetchJson(`https://api.vhtear.com/randomwibu&apikey=${Vhtear}`)
					reply(ind.wait())
			...
			if (!isGroup) return reply(ind.groupo()) 
              	    if (args.length < 1) return reply('teksnya mana gan?')
                    teks = body.slice(9)
                    anu = await fetchJson(`https://api.vhtear.com/nekosearch?query=${teks}&apikey=${Vhtear}`, {method: 'get'})
			reply(ind.wait())
                    teks = `===============\n`
		...
		case 'resepmasakan':  
                ...
                if (isLimit(sender)) return reply(ind.limitend(pushname))
                reply(ind.wait)
                   anu = await fetchJson(`https://api.vhtear.com/resepmasakan?query=${body.slice(12)}&apikey=${Vhtear}`, {method: 'get'})
                   ...
                   if (isLimit(sender)) return reply(ind.limitend(pusname))
                     teks = body.slice(9)
                     anu = await fetchJson(`https://api.vhtear.com/igprofile?query=${teks}&apikey=${Vhtear}`, {method: 'get'})
	  ...
	  case 'seberapagay':
		...
		if (!isGroup) return reply(ind.groupo())                 
				...
				mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					...
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pushname))
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query={body.slice(11)}&apikey=${Vhtear}`, {method: 'get'})
	...
	case 'hobby':
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname))
		if (!isGroup) return reply(ind.groupo())
					anu = await fetchJson(`https://dapuhy-apikey.herokuapp.com/api/kuis/caklontong?apikey=${DapApi}`, {method: 'get'})
		...
		if (!isGroup) return reply(ind.groupo())
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://dapuhy-apikey.herokuapp.com/api/kuis/tebakgambar?apikey=${DapApi}`, {method: 'get'})
...
🌹⃟🔰 -----[ *MEDIA MENU* ]----- 🔰⃟🌹
Hola, ${pushname} 👋
Ojala este bot sea de su gusto User, ${sender.split("@")[0]}
┏━━━━━━━━━━━━━━━━━━━━┓
┃╭───────────────────
┃│➸ *${prefix}brainly*
┃│➸ *${prefix}jadwaltv*
...
*YT = 
*IG =
*GC = https://chat.whatsapp.com/F2vCvhtry6P0sZa08CXsVC
*OWNER = Wa.me/+523131134857
